...
end_of_timestep_observer = function(data, vars, config){
  ...
  save_species()
  save_landscape()
  print(mean(data$landscape$environment))
}
...
create_ancestor_species <- function(landscape, config) {
  ...
  opt_temps <- c(10, 11, 12)
  nich_widths <- c(0.5, 0.9, 1)
  disps <- c(200, 60, 100)
  new_species <- list()
  for(i in 1:3){
...
get_dispersal_values <- function(n, species, landscape, config) {
  # browser()
  values <- rep(100, n)
  # values <- rep(species$traits[1, "dispersal"], n)
...
apply_ecology <- function(abundance, traits, landscape, config, abundance_scale = 10, abundance_threshold = 1) {
  ...
  ############# INTERACT ################
  # To undertand this... play with the ecology function and see what is happening, for example...
